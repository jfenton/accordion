...
function driverMySQLGetData($link, $objName, $fields, $lmField, $lm) {
	$selectFields = join(",", $fields);
	$sql = "SELECT {$selectFields} FROM {$objName} WHERE {$lmField} > {$lm} ORDER BY {$lmField} DESC";
	print $sql."\n";
...
foreach($objs as $objname => $obj) {
	...
	if($obj->{'src'} === "sf") {
		...
		$dataRaw = driverSalesforceGetData($sf, $bac, $obj->{'srcTable'}, $selectFields, $lm);
		$isFirstRow = true;
		$fieldsToNames = array();
	...
	foreach($obj->{'fields'} as $field) {
		...
		if($numpipes == 2) {
			...
			if($filterCmd == 'LOOKUP') {
				...
				$sql = "SELECT ".$lookupUsing.",".$thenUse." FROM ".$lookupTable." WHERE ".$lookupUsing." IN (".join(",", $lookupFieldValues).")";
				$stmt = mysqli_query($link, $sql);
				while($result = mysqli_fetch_array($stmt, MYSQLI_ASSOC)) {
			...
			// Simple - direct mapping of field to equal field name on opposing engine
			preg_match_all("/^(.*)->(.*)$/", $field, $pipes, PREG_SET_ORDER);
			$srcField = $pipes[0][1];
			$dstField = $pipes[0][2];
			foreach($data as &$datum) {
	...
	if(count($errors) == 0) {
		logger(2, 'No errors to upload to Salesforce.');
	} else {
		$csvfh = fopen('php://temp/maxmemory:'. (256*1024*1024), 'r+'); // 256MB RAM, otherwise back to file
		...
		logger(2, 'Uploading errors to Salesforce');
